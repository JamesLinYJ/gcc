...
;; A bare symbol reference can reach movsi_internal through late
;; constant rematerialization, which does not go through the movsi
;; expander and its GOT loading.  Under FDPIC the address must come
;; from the GOT: an absolute reference would need a runtime relocation
